...
# =========================
# Default profiles (explicit in code)
# =========================
...
def levenshtein_distance_items(a: List[str], b: List[str]) -> int:
    # Prefer item-level distance via seqdistance if available
    if hasattr(Levenshtein, "seqdistance"):
        return int(Levenshtein.seqdistance(a, b))
    # Fallback: string distance on a safe separator (still item-granular due to separator boundaries)
    sep = "␟"
    return Levenshtein.distance(sep.join(a), sep.join(b))
...
def compare_one(input_codes: List[str], ref_codes: List[str], saga: str) -> dict:
    ...
    score = (s_lev + s_w) / 2.0 if (s_lev == s_lev and s_w == s_w) else s_lev if s_w != s_w else s_w
    return {
        "levenshtein": d,
...
def main():
    ap = argparse.ArgumentParser(description="Compara um ranking (HP ou SW) com Ale e Leo usando Levenshtein norm e WMAPE.")
    ap.add_argument("--saga", required=True, choices=["hp","sw"], help="Saga do ranking de entrada (hp ou sw).")
    ap.add_argument("--ranking", required=True, help="Ranking como string separada por vírgulas (códigos ou títulos). Ex.: 'HP2,HP4,HP1,...'")
    args = ap.parse_args()

    # perfis explícitos no código
    hp_ale, hp_leo = DEFAULT_HP_ALE, DEFAULT_HP_LEO
    sw_ale, sw_leo = DEFAULT_SW_ALE, DEFAULT_SW_LEO
